...
public class SVPinView: UIView {
    ...
    @IBInspectable public var interSpace: CGFloat = 10
    @IBInspectable public var textColor: UIColor = UIColor.black
    @IBInspectable public var placeholder: String = ""
    
    @IBInspectable public var secureCharacter: String = "*"
    @IBInspectable public var shouldSecureText: Bool = true
    @IBInspectable public var secureTextDelay: Int = 250
    
    @IBInspectable public var borderLineColor: UIColor = .lightGray
    ...
    @IBInspectable public var activeBorderLineThickness: CGFloat = 3
    ...
    public var font: UIFont = .boldFontWithSize(size: 22)
    ...
    fileprivate func stylePinField(containerView: UIView, underLine: UIView, isActive: Bool) {
        ...
        func setupUnderline(color: UIColor, withThickness thickness: CGFloat) {
            ...
            underLine.layer.cornerRadius = thickness / 2
            underLine.constraints.filter { ($0.identifier == "underlineHeight") }.first?.constant = thickness
        }
        
        if isActive { setupUnderline(color: activeBorderLineColor, withThickness: activeBorderLineThickness) } else { setupUnderline(color: borderLineColor, withThickness: borderLineThickness) }
        containerView.layer.borderWidth = 0
        containerView.layer.borderColor = UIColor.clear.cgColor
    }
    
    fileprivate func refreshPinView(completionHandler: (() -> Void)? = nil) {
...
extension SVPinView: UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
    ...
    public func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        ...
        let width = (self.bounds.width - (interSpace * CGFloat(max(pinLength, 1) - 1)))/CGFloat(pinLength)
